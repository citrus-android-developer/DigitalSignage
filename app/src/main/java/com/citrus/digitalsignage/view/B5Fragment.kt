...
import com.citrus.digitalsignage.databinding.FragmentB5Binding
...
@AndroidEntryPoint
class B5Fragment : BaseFragment<FragmentB5Binding>(FragmentB5Binding::inflate) {
    private val sharedViewModel: SharedViewModel by activityViewModels()
     ...
     override fun initView() {
        ...
        binding.p4MenuView?.visibility = View.GONE
    }
     ...
     override fun initObserver() {
            ...
            when(status){
                is BlockStatus.StatusInfo -> {
                    when(status.type){
                        ...
                        ShowType.BANNER -> {
                            showBanner(binding.p1BannerView as Banner<DataBean, BannerImageAdapter<DataBean>>,status.param as List<DataBean>)
                        }
                        ShowType.IMG -> {
            ...
            when(status){
                is BlockStatus.StatusInfo -> {
                    when(status.type){
                        ...
                        ShowType.BANNER -> {
                            showBanner(binding.p2BannerView as Banner<DataBean, BannerImageAdapter<DataBean>>,status.param as List<DataBean>)
                        }
                        ShowType.IMG -> {
            ...
            when(status){
                is BlockStatus.StatusInfo -> {
                    when(status.type){
                        ...
                        ShowType.BANNER -> {
                            showBanner(binding.p3BannerView as Banner<DataBean, BannerImageAdapter<DataBean>>,status.param as List<DataBean>)
                        }
                        ShowType.IMG -> {
        ...
        sharedViewModel.p4.observe(viewLifecycleOwner,{ status ->
            when(status){
                is BlockStatus.StatusInfo -> {
                    when(status.type){
                        ShowType.VIDEO -> {
                            showVideo(binding.p4YoutubeView, status.param as String)
                        }
                        ShowType.URL -> {
                            showWebView(binding.p4WebView,status.param as String)
                        }
                        ShowType.BANNER -> {
                            showBanner(binding.p4BannerView as Banner<DataBean, BannerImageAdapter<DataBean>>,status.param as List<DataBean>)
                        }
                        ShowType.IMG -> {
                            showMenu(binding.p4MenuView,status.param as String)
                        }
                    }
                }
            }
